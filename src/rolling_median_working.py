...
with open("../data-gen/venmo-trans.txt", 'r') as ven:
	...
	G.add_edge(*pairs[0], time=max_time)
...
#		print(dd.items())
		...
		G.add_edge(*pairs[len(pairs)-1], time=datetime.datetime.strptime(d["created_time"], "%Y-%m-%dT%H:%M:%SZ"))	#add just latest pair
		print(G.number_of_nodes())
		print(G.number_of_edges())
		print('#of connections to latest node: ',len(G.neighbors(pairs[len(pairs)-1][1])))
