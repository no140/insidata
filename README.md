Language: Python3
This is a simple script that attempts to follow guidelines (below), by reading the transaction input file, line by line, adding/subtracting appropriate transactions to the plot, and listing a median degree number to the output file.
All transaction info lies within the graphing module used here: NetworkX. Some other, supposedly faster,  graphing modules were tried, but either did not contain clean methods for holding and graphing the data as necessitated by this exercise, or had problems running on my machine. Unfortunately NetworkX doesn't seem to have a way to draw one edge at a time, meaning that each plot draw is a full redraw, and causing the program to more than double the time it takes to complete a dataset. (a difference of 160 seconds vs. 15-60 seconds, depending on detail within plot, on dataset of 1700 lines)
...
Re-edit : also uploaded a noplot version not based on NetworkX. Slightly faster, but shows slightly different output medians than previous version, may be a bug within NetworkX, maybe not.

Modules: json, datetime, collections.defaultdict, numpy, sys
